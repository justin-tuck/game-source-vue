<template>
    ...
    <div  v-show="!loading">
        <v-table theme="dark">
            ...
            <tbody>
                <tr v-for="article in articleStore.adminArticles">
                    ...
                    <td>
                        <v-btn variant="outlined" color="yellow" size="small" @click="router.push({name:'admin_edit', params:{id:article.id}})">
                            Edit
                        </v-btn>
                    </td>
                </tr>
        ...
        <div class="text-center" v-if="btnLoad">
            <v-progress-circular indeterminate color="primary" />
        </div>
        <v-btn v-else
        variant="outlined" class="mt-3" @click="loadMoreHandler()">
        Get more articles
        </v-btn>
    </div>
</template>
